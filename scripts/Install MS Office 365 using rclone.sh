...
service_acct () {
  SERVICE=/Users/Shared/service/json-file-name.json
  if test -f "$SERVICE"; then
        ...
        tee -a /Users/Shared/service/json-file-name.json << 'END'
{
  "type": "service_account",
  "project_id": "json-file-name",
  "private_key_id": "privkey",
  "private_key": "here",
  "client_email": "rclone@json-file-name.iam.gserviceaccount.com",
  "client_id": "client_id",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/rclone%40json-file-name.iam.gserviceaccount.com"
}
END
  fi
}
...
scope = drive
service_account_file = /Users/Shared/service/json-file-name.json
team_drive =
END
    fi
}
...
LOCK=/tmp/lock.file
if [ -f "$LOCK" ]; then
    echo "$LOCK exists, not doing actions";
else
    touch $LOCK;
...
filename="Microsoft_Office_16.60.22041000_BusinessPro_Installer.pkg"
APP="MS Office 365"

TEMP="/Library/Application Support/Microsoft/MAU2.0/Microsoft AutoUpdate.app"
TEM="/Applications/Microsoft Word.app"
if [ -d "$TEMP" -a -d "$TEM" ]; then
    echo "$APP is already installed on $(hostname)"
elif [ ! -f "${filepath:?}/${filename}" ]; then
    echo "Install file not found, not installing ${filepath:?}/${filename}..."
